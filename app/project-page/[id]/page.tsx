"use client";
import ProjectData from "@/data/projectData"; // adjust path accordingly
...
const ProjectPage = () => {
  ...
  const nextProject = () => {
    const nextId = id + 1;
    const nextProject = ProjectData.find((p) => p.id === nextId);

    if (nextProject) {
      router.push(`/project-page/${nextId}`);
    }
  };

  const prevProject = () => {
    const prevId = id - 1;
    const prevProject = ProjectData.find((p) => p.id === prevId);

    if (prevProject) {
      router.push(`/project-page/${prevId}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900">
      ...
      >
        ...
        >
          ...
          {project.id + 1 < ProjectData.length && (
            <motion.button
              className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors"
              whileHover={{ x: 5 }}
              onClick={() => nextProject()}
